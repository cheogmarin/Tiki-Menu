...
-- Tabla de Mesas (Tokens de seguridad)
...
CREATE TABLE IF NOT EXISTS llamados (
  ...
  mesa_numero TEXT NOT NULL, -- Guardamos el número para facilitar la vista del staff
...
-- Permitir lectura pública de mesas (para validar token en el cliente)
CREATE POLICY "Permitir lectura pública de mesas" ON mesas FOR SELECT USING (true);

-- Permitir insertar llamados solo si el mesa_id es válido
CREATE POLICY "Insertar llamados con token válido" ON llamados FOR INSERT 
WITH CHECK (EXISTS (SELECT 1 FROM mesas WHERE id = mesa_id));

-- Permitir al staff ver todos los llamados
CREATE POLICY "Staff puede ver llamados" ON llamados FOR SELECT USING (true);
CREATE POLICY "Staff puede actualizar llamados" ON llamados FOR UPDATE USING (true);

-- Función Segura para Llamar al Mesonero (Bloqueo inteligente)
CREATE OR REPLACE FUNCTION llamar_mesonero_seguro(p_token UUID)
RETURNS JSON AS $$
DECLARE
  v_mesa_numero TEXT;
  v_ultimo_llamado TIMESTAMP WITH TIME ZONE;
  v_resultado JSON;
BEGIN
  -- 1. Validar Token
  SELECT numero INTO v_mesa_numero FROM mesas WHERE id = p_token;
  
  IF v_mesa_numero IS NULL THEN
    RETURN json_build_object('success', false, 'message', 'Token de mesa inválido.');
  END IF;

  -- 2. Verificar si hay un llamado pendiente en los últimos 180 segundos
  SELECT created_at INTO v_ultimo_llamado 
  FROM llamados 
  WHERE mesa_id = p_token 
    AND estado = 'pendiente'
    AND created_at > (NOW() - INTERVAL '180 seconds')
  ORDER BY created_at DESC 
  LIMIT 1;

  IF v_ultimo_llamado IS NOT NULL THEN
    RETURN json_build_object(
      'success', false, 
      'already_called', true,
      'message', '¡Atención! Un compañero de tu mesa ya ha solicitado un mesonero. Ya estamos en camino.'
    );
  END IF;

  -- 3. Insertar nuevo llamado
  INSERT INTO llamados (mesa_id, mesa_numero) 
  VALUES (p_token, v_mesa_numero);

  RETURN json_build_object('success', true, 'message', 'Llamado enviado con éxito.', 'mesa', v_mesa_numero);
END;
